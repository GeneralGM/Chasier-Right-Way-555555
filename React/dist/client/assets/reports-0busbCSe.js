import{c as P,u as L,r as M,a as l,j as e,B as E,k as O,f as u}from"./index-BMx-8oXP.js";import{u as W}from"./pos-store-CazYM2D-.js";import{P as G}from"./printer-BQcbHICN.js";import{C as H}from"./chef-hat-Bl2M6s_6.js";import{W as q,C as Z,S as U,B as J,D as R,P as K,M as Q,R as X,a as Y}from"./wine-5fV_4pll.js";const ee=[["path",{d:"m16 17 5-5-5-5",key:"1bji2h"}],["path",{d:"M21 12H9",key:"dn1m92"}],["path",{d:"M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4",key:"1uf3rs"}]],te=P("log-out",ee);const ae=[["path",{d:"M4 14a1 1 0 0 1-.78-1.63l9.9-10.2a.5.5 0 0 1 .86.46l-1.92 6.02A1 1 0 0 0 13 10h7a1 1 0 0 1 .78 1.63l-9.9 10.2a.5.5 0 0 1-.86-.46l1.92-6.02A1 1 0 0 0 11 14z",key:"1xq2db"}]],se=P("zap",ae);function ne(){const{db:h}=L(),{db:r,closeShift:f}=W(),n=M.useMemo(()=>{if(!r.shift)return[];const o=localStorage.getItem("isSecCashierDevice")==="true"?"Sub-1":"Main";return r.invoices.filter(c=>c.createdAt>=r.shift.openedAt&&(c.terminalId===o||c.terminal_id===o))},[r.invoices,r.shift]),t=M.useMemo(()=>{let a=0,o=0,c=0,g=0,p=0,x=0,m=0,b=0,j=0,w=0,k=0,C=0,S=0,T=0,y=0;for(const s of n){m+=s.subtotal,b+=s.discountValue,j+=s.taxValue||0;const v=Number(s.deliveryPrice)||Number(s.delivery_price)||0;x+=v,s.type==="takeaway"?g+=s.total-v:s.type==="delivery"&&(p+=s.total-v);const F=s.orderCategory||s.order_category||"normal";F==="fast"?(w+=s.total,k++,C+=Number(s.commissionValue||s.commission_value||0)):F==="talabat"&&(S+=s.total,T++,y+=Number(s.commissionValue||s.commission_value||0));for(const d of s.items){const V=d.extras?.reduce((z,A)=>z+Number(A.price||0),0)||0,_=(Number(d.unitPrice||d.price||0)+V)*Number(d.qty||1);let D=d.department||"مطبخ";const N=h.meals.find(z=>z.id===d.mealId);N&&(N.department==="شيشه"||(N.category||"").trim().replace("ة","ه")==="شيشه"?D="شيشة":N.department&&(D=N.department)),D.includes("شيش")?c+=_:D.includes("بار")?o+=_:a+=_}}const $=a+o+y+c+j-b;return{kitchen:l(a),bar:l(o),shisha:l(c),takeaway:l(g),deliveryTotal:l(p),deliveryFees:l(x),subtotal:l(m),discount:l(b),tax:l(j),total:l($),revenues:l($),fastTotal:l(w),fastCount:k,fastComission:l(C),talabatTotal:l(S),talabatCount:T,talabatCommission:l(y)}},[n,h.meals]);async function I(){const a=prompt("برجاء إدخال مبلغ الكاش الفعلي الموجود بالدرج حالياً لتسوية العهدة:");if(a===null)return;const o=Number(a)||0,g=localStorage.getItem("isSecCashierDevice")==="true"?"Sub-1":"Main",p=await f({kitchenSales:t.kitchen,barSales:t.bar,shishaSales:t.shisha,taxValue:t.tax,discountValue:t.discount,dineinSales:t.revenues-t.takeaway-t.deliveryTotal,takeawaySales:t.takeaway,deliverySales:t.deliveryTotal,fastSales:t.fastTotal,talabatSales:t.talabatTotal,talabatCommission:t.talabatCommission,terminalId:g,actualCash:o});if(p){if(p.auditReport){const x=p.auditReport,m=x.variance;let b="متطابق 🟢";m<0?b=`عجز: ${Math.abs(m).toFixed(2)} ج.م 🔴`:m>0&&(b=`زيادة: ${m.toFixed(2)} ج.م 🔵`),t.revenues-t.takeaway-t.deliveryTotal;const j=t.kitchen,w=t.bar,k=t.shisha,C=t.takeaway,S=t.deliveryTotal,T=t.tax,y=t.discount,$=t.revenues,s=t.deliveryFees,v=new Date(r.shift.openedAt).toLocaleString("ar-EG"),F=new Date().toLocaleString("ar-EG"),d=window.open("","_blank","width=600,height=800");d&&(d.document.write(`
            <html dir="rtl">
              <head>
              ...
              <body onload="setTimeout(() => { window.print(); window.close(); }, 500);">
                ...
                <div class="header-box">
                  ...
                  <div class="header-row"><span>من:</span><span dir="ltr">${v}</span></div>
                ...
                <table>
                  ...
                  <tr><td>بار</td><td class="bold">${w.toFixed(2)}</td></tr>
                  <tr><td>شيشة</td><td class="bold">${k.toFixed(2)}</td></tr>
                </table>
                  
                <table>
                  <tr><td colspan="2" class="table-header">توزيع الطلبات والمنصات</td></tr>
                  <tr><td>تيك اواي</td><td class="bold">${C.toFixed(2)}</td></tr>
                  <tr><td>دليفري</td><td class="bold">${S.toFixed(2)}</td></tr>
                ...
                <table>
                  ...
                  <tr><td>إجمالي الضريبة</td><td class="bold">${T.toFixed(2)}</td></tr>
                  <tr><td>إجمالي الخصم</td><td class="bold">${y.toFixed(2)}</td></tr>
